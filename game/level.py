...
def create_room(width=10.0, height=4.0, depth=10.0) -> Scene:
    ...
    scene.add_entity(Entity(cube, position=(W/2, 0, -D/1.8), scale=(T, H*2, 10), color=(0.7, 0.3, 0.3)))
    # Divider wall Right - red
    scene.add_entity(Entity(cube, position=(W/2, 0, D/1.05), scale=(T, H*2, D/8), color=(0.7, 0.3, 0.3)))
    # Divider ceiling - red
    scene.add_entity(Entity(cube, position=(W*0.9, H*1.8, 1.5), scale=(T, H*0.1, D/8), color=(0.7, 0.3, 0.3)))
